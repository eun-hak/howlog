...
export async function generateMetadata({
  ...
  return {
    title: `${category.name} - 하우로그`,
    description: category.description,
    keywords: [category.name, category.slug, "블로그", "카테고리", "하우로그"],
    openGraph: {
      title: `${category.name} - 하우로그`,
      description: category.description,
      type: "website",
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const category = CATEGORIES.find((c) => c.slug === slug);
  const posts = POSTS.filter((p) => p.category === slug).sort((a, b) =>
        ...
        <div className="w-full md:w-80 shrink-0 space-y-8">
          ...
          <div className="bg-gray-50 p-6 rounded-lg border border-gray-100">
            ...
            <p className="text-sm text-gray-600 leading-relaxed">
              {category.description} 실용적인 가이드와 노하우를 통해 도움을
              드립니다. 이 섹션은 정기적으로 새로운 콘텐츠로 업데이트됩니다.
            </p>
          </div>
        </div>
      </div>
